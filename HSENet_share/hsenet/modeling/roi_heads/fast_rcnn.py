...
def fast_rcnn_inference_single_image(
    ...
    #filter_mask = scores > torch.tensor(score_thresh).cuda()  # R x K
    filter_mask = scores > torch.tensor(score_thresh).to(scores.device)  # R x K
...
class FastRCNNOutputs:
    ...
    def __init__(
        ...
        if len(proposals):
            ...
            if proposals[0].has("gt_classes"):
                ...
                self.gt_sources = cat([p.gt_dataset_source for p in proposals], dim=0)
    ...
    def softmax_cross_entropy_loss(self):
        ...
        if self._no_instances:
            ...
            loss = 0.0 * self.pred_class_logits.sum()
            #self._log_accuracy()
            #if gt_source.sum():
            gt_source = self.gt_sources==0
            if gt_source.any():
                loss += F.cross_entropy(self.pred_class_logits[gt_source], self.gt_classes[gt_source], reduction="mean")
                # return F.cross_entropy(self.pred_class_logits, self.gt_classes, reduction="mean")

            # If COCO instance segmentation dataset
            # Useful class {Person:0, Car:2, motorbike:3, }
            #gt_source = self.gt_sources==3
            #if gt_source.any():
            #    import ipdb
            #    ipdb.set_trace()

            if len(self.gt_classes[~gt_source][self.gt_classes[~gt_source]==0])>0:
                loss += F.cross_entropy(self.pred_class_logits[~gt_source][self.gt_classes[~gt_source]==0] , self.gt_classes[~gt_source][self.gt_classes[~gt_source]==0], reduction="mean")
...
class FastRCNNOutputLayers(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        #self.cls_score = Linear(input_size, 1 + 1)  # objectness and background
    ...
    def predict_boxes_for_gt_classes(self, predictions, proposals):
        ...
        if K > 1:
            ...
            gt_classes = gt_classes.clamp_(1, K)
